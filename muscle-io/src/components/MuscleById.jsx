...
function MuscleById() {
    ...
    return (
        <div className="inset-0 -z-10 w-full h-screen flex flex-col items-center justify-center [background:radial-gradient(125%_125%_at_50%_10%,#000_40%,#63e_100%)]">

            
            {data.map((item, index) => (
                <div key={index} className="mt-20 p-2 flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-6xl">

                    <img className="rounded-lg w-full" src={item.gifUrl}></img>

                    <div className="flex flex-col justify-between p-4 leading-normal">

                        <h4 className="text-3xl font-bold tracking-tight text-[#160B34] mb-8">{item.name.charAt(0).toUpperCase() + item.name.slice(1)}</h4>
